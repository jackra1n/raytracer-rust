...
impl Hittable for Cube {
    fn hit(&self, ray: &Ray, t_min: f32, t_max: f32) -> Option<HitRecord> {
        ...
        let ray_origin_obj_h = self.world_to_object
            * Vec4::new(
                ray_origin_crate.x,
                ray_origin_crate.y,
                ray_origin_crate.z,
        ...
        let ray_direction_obj_h = self.world_to_object
            * Vec4::new(
                ray_direction_crate.x,
                ray_direction_crate.y,
                ray_direction_crate.z,
                0.0,
            );

        let ray_origin_obj = Vec3::new(ray_origin_obj_h.x, ray_origin_obj_h.y, ray_origin_obj_h.z);
        let ray_direction_obj = Vec3::new(
        ...
        let position_world_h =
            self.object_to_world * Vec4::new(position_obj.x, position_obj.y, position_obj.z, 1.0);
        let position_world =
            CrateVec3::new(position_world_h.x, position_world_h.y, position_world_h.z);
